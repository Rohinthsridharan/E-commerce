...
const ArtistDashboard = () => {
  ...
  useEffect(() => {
    const loggedInArtistId = localStorage.getItem("artistId"); // ✅ Get logged-in artistId
    const storedOrders = JSON.parse(localStorage.getItem("orders")) || [];
    const storedPayments = JSON.parse(localStorage.getItem("paymentRequests")) || [];

    // ✅ Filter only the orders that belong to the logged-in artist
    const artistOrders = storedOrders.filter((order) => order.artistId === loggedInArtistId);

    // ✅ Map orders to payments (Apply discounted price if available)
    const updatedOrders = artistOrders.map(order => {
      const matchingPayment = storedPayments.find(payment => payment.id === order.id);
      return {
        ...order,
        finalPrice: matchingPayment ? matchingPayment.amount : order.total, // ✅ Use discounted price if available
      };
    });

    setOrders(updatedOrders);

    // ✅ Fetch and calculate revenue from payments
    const revenue = storedPayments
      .filter((payment) => payment.status === "Completed" && payment.artistId === loggedInArtistId)
                    ...
                    <tbody>
                      {orders.slice(-5).reverse().map((order) => (
                        <tr key={order.id}>
                          ...
                          <td>₹{order.finalPrice}</td> {/* ✅ Show final price after payment updates */}
                          <td>
